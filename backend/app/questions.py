...
class QuestionGenerator:
    ...
    def sample_alt(
        ...
        eligible_groups = [
            ...
            if obscurity_lo <= int(g.get("obscurity", 99)) <= obscurity_hi
        ]

        # Sample uniformly from every concrete question that matches the requested
        # metrics. Do not first choose an airline group, country, continent, or
        # geography-derived bucket: doing so would make regions with more/smaller
        # groups overrepresented even when many endpoint pairs elsewhere satisfy
        # the same conn_tier, n_stops, and obscurity constraints.
        chosen_question: tuple[str, str, str, int, int] | None = None
        seen_questions = 0
        for gid in _shuffled(eligible_groups, rng):
            adj, _edge_airlines = self._subgraph(gid)
            tier_nodes = [code for code in endpoint_nodes if code in adj]
            if len(tier_nodes) < 2:
                continue
            for (a_iata, b_iata), hops in self._pairs_with_hop_range(
                adj,
                tier_nodes,
                min_hops=min_hops,
                max_hops=max_hops,
                ...
                seen_questions += 1
                if rng.randrange(seen_questions) == 0:
                    chosen_question = (gid, a_iata, b_iata, conn_tier_for_question, hops)
